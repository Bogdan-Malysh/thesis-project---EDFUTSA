...
import arima_validation_2024 as validation_2024
...
AUTHORITATIVE_SELECTION_PATH = TABLE_DIRECTORY / "arima_selected_2024.csv"
AUTHORITATIVE_SHORTLIST_PATH = TABLE_DIRECTORY / "arima_shortlists_2024.csv"
...
JOB_KEY_COLUMNS = list(validation_2024.ARIMA_JOB_KEY)
...
def verify_authoritative_selection(
    ...
    if missing:
        raise ValueError(f"authoritative ARIMA selection is missing columns: {missing}")
    ...
    if len(selected) != len(COUNTRY_CONFIG) or set(selected["country"]) != set(COUNTRY_CONFIG):
        raise ValueError("authoritative selection must contain arima_p2_d1_q2 for both countries")
    if not selected["expected_jobs"].eq(366).all():
        raise ValueError("authoritative 2024 ARIMA selection must expect 366 jobs per country")
    if not selected["completed_jobs"].eq(366).all():
        raise ValueError("authoritative 2024 ARIMA selection is incomplete")
    if not selected["failed_jobs"].eq(0).all() or not selected["coverage"].eq(1.0).all():
        raise ValueError("authoritative 2024 ARIMA selection contains failures or incomplete coverage")
    ...
    arima_registry = registry.loc[registry["model_family"].eq(TABLE_MODEL_FAMILY)].copy()
    if len(arima_registry) != len(COUNTRY_CONFIG) or set(arima_registry["country"]) != set(COUNTRY_CONFIG):
        raise ValueError("selected-model registry must contain one ARIMA row per country")
    if not arima_registry["selected_specification_id"].eq(SPECIFICATION_ID).all():
        raise ValueError("selected-model registry does not select arima_p2_d1_q2 for both countries")
...
def load_test_shortlist(
    ...
    required = {"country", "specification_id", "p", "d", "q", "trend"}
    ...
    selected = shortlist.loc[shortlist["specification_id"].eq(SPECIFICATION_ID)].copy()
    if len(selected) != len(COUNTRY_CONFIG) or set(selected["country"]) != set(COUNTRY_CONFIG):
        raise ValueError("ARIMA shortlist must contain arima_p2_d1_q2 for both countries")
    if not selected["p"].eq(ORDER[0]).all() or not selected["d"].eq(ORDER[1]).all():
        raise ValueError("ARIMA shortlist has the wrong p or d order")
    if not selected["q"].eq(ORDER[2]).all() or not selected["trend"].eq(TREND).all():
        raise ValueError("ARIMA shortlist has the wrong q order or trend")
...
def test_dates(frame: pd.DataFrame) -> list[date]:
    ...
    if values != expected:
        raise ValueError(
            "2025 ARIMA test requires every local date from 2025-01-01 through 2025-12-31"
        )
    return values
...
def build_test_manifest(
    ...
    if not dates or any(
        ...
        raise ValueError("ARIMA test manifest requires non-empty 2025 target dates")
    ...
    for country in COUNTRY_CONFIG:
        ...
        for target_date in dates:
            ...
            if key in seen:
                raise ValueError(f"duplicate ARIMA test job: {key}")
            seen.add(key)
            rows.append(
                {
                    "country": country,
...
def expected_training_observations(job: pd.Series | dict[str, object], frame: pd.DataFrame) -> int:
    origin = country_forecast_origin(job["target_date"], str(job["country"]))
    return len(information_set(frame, origin))


def _base_job_row(job: dict[str, object], context: object, expected_observations: int) -> dict[str, object]:
    return {
...
def _execute_job_2025(job: dict[str, object], frame: pd.DataFrame) -> dict[str, object]:
    country = str(job["country"])
    target_date = str(job["target_date"])
    ...
    job_row = _base_job_row(job, context, len(target))
    job_row["training_observations"] = len(available)

    fit = fit_arima(available["actual_load_mwh"].to_numpy(dtype=float), ORDER)
    job_row.update(validation_2024._fit_metadata(fit))
    ...
    try:
        path = validation_2024.build_arima_forecast_path(
        ...
        job_row.update(
            {
                ...
                "bridge_intervals": int((~path["is_target_day"]).sum()),
...
def validate_job_result(
    ...
    if forecast.empty:
        raise ValueError("completed ARIMA job has no forecast rows")
    country = str(job_row["country"])
    target_date = str(job_row["target_date"])
    if str(job_row["specification_id"]) != SPECIFICATION_ID:
        raise ValueError("ARIMA result has the wrong specification")
    if not forecast["model_family"].eq(MODEL_FAMILY).all():
        raise ValueError("ARIMA result has the wrong model family")
    if not forecast["specification_id"].eq(SPECIFICATION_ID).all():
        raise ValueError("ARIMA result has the wrong specification label")
    if not forecast["target_date"].eq(target_date).all():
        raise ValueError("ARIMA result has the wrong target date")
    ...
    if target.empty or not target["local_date"].eq(target_date).all():
        raise ValueError("ARIMA result does not contain the complete target day")
    if len(target) not in (23, 24, 25):
        raise ValueError("ARIMA target day has an invalid DST interval count")
    if target["timestamp_utc"].duplicated().any():
        raise ValueError("ARIMA target day contains duplicate UTC timestamps")
    if not forecast["local_date"].astype(str).le(target_date).all():
        raise ValueError("ARIMA path contains a future local date")
...
def _normalize_forecasts(frame: pd.DataFrame) -> pd.DataFrame:
    normalized = _normalize_keys(frame, JOB_KEY_COLUMNS)
    if not normalized.empty and "timestamp_utc" in normalized:
        ...
        ).map(lambda value: value.isoformat() if pd.notna(value) else value)
    return normalized


def _upsert(existing: pd.DataFrame, incoming: pd.DataFrame, keys: list[str]) -> pd.DataFrame:
...
class TestValidationStore:
    ...
    def persist_result(self, job_row: dict[str, object], forecast: pd.DataFrame) -> None:
        if str(job_row.get("status", "")) == "completed" and forecast.empty:
            raise ValueError("completed ARIMA jobs must include forecast rows")
        jobs = _upsert(
            self.read_jobs(),
...
def _summary_frame(
    ...
    return validation_2024._summary_frame(jobs, manifest, forecasts)
...
def _table_rows(summary: pd.DataFrame) -> pd.DataFrame:
    ...
    if missing:
        raise ValueError(f"ARIMA test summary is missing required columns: {missing}")
    rows = summary.loc[
...
def _atomic_write(frame: pd.DataFrame, path: Path, columns: list[str]) -> None:
...
def update_test_tables(
    ...
    overview_frame = _merge_table(_read_existing_table(overview), incoming)
    _atomic_write(all_models, all_path, list(all_models.columns))
    _atomic_write(overview_frame, overview, list(overview_frame.columns))
    return {"all_models": all_path, "overview": overview}
...
def validate_aggregate(
    ...
    if len(job_frame) != 2 * EXPECTED_DAYS or not job_frame["status"].eq("completed").all():
        raise ValueError("ARIMA test does not contain 730 completed jobs")
    if job_frame.duplicated(JOB_KEY_COLUMNS).any():
        raise ValueError("ARIMA test contains duplicate job keys")
    for country, frame in frames.items():
        ...
        if len(country_jobs) != EXPECTED_DAYS:
            raise ValueError(f"{country} does not contain 365 ARIMA jobs")
        ...
        if not training.is_monotonic_increasing:
            raise ValueError(f"ARIMA training history is not expanding for {country}")
        for row in country_jobs.to_dict("records"):
            ...
            if int(row["training_observations"]) != expected:
                raise ValueError(f"ARIMA training count is not origin-bounded for {country}")
        ...
        if len(target) != EXPECTED_OBSERVATIONS:
            raise ValueError(f"{country} does not have 8760 ARIMA target observations")
        if target["timestamp_utc"].duplicated().any():
            raise ValueError(f"duplicate ARIMA target timestamps for {country}")
        for target_date, expected_count in (("2025-03-30", 23), ("2025-10-26", 25)):
            if int(target["local_date"].eq(target_date).sum()) != expected_count:
                raise ValueError(f"invalid ARIMA DST count for {country} {target_date}")
...
def run_test(
    ...
    if workers != WORKERS:
        raise ValueError("the approved ARIMA 2025 test requires exactly 4 workers")
    verify_authoritative_selection()
    load_test_shortlist(shortlist_path)
    frames = {
        country: load_country_data(country, processed_directory)
        for country in COUNTRY_CONFIG
    }
    ...
    if len(manifest) != 2 * EXPECTED_DAYS:
        raise ValueError(f"expected 730 ARIMA test jobs, found {len(manifest)}")
    ...
    pending = [
        ...
        if (row["country"], row["target_date"], row["specification_id"]) not in completed_keys
    ]
    ...
    if pending:
        ...
        with ProcessPoolExecutor(
            ...
            for result in executor.map(_worker_execute, pending):
                ...
                store.write_summary(
                    build_test_summary(
                        store.read_jobs(), manifest, store.read_forecasts()
                    )
                )
    ...
    if completed != 2 * EXPECTED_DAYS or failed != 0:
        raise RuntimeError(
            f"ARIMA 2025 test incomplete: {completed}/730 completed, {failed} failed"
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the final 2025 ARIMA test only")
